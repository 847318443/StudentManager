            ...
            if (flags == "1")
            {
                ...
                for (int i = 0; i < checkedListBoxtime.Items.Count; i++)
                {
                    if (checkedListBoxtime.GetItemChecked(i))
                    {
                        ...
                        randno();
                        sql = "insert into Ctime(Cname,Cterm,nteacher,CTid,Ctime,Classroom,compus,Ccapacity,Cselected) values('" + classes + "','" + term + "','" + teacher + "','" + no + "','" + time + "','" + didian + "','" + xiaoqu.SelectedItem.ToString().Trim() + "','" + textBox1.Text + "',0)";
                        cmd.CommandText = sql;
                        cmd.ExecuteNonQuery();
                    }
                }
